...
urls.each do |url|
  ...
  unless (JSON.parse(result)["rows"][0]["elements"][0]["duration"]["value"] rescue nil).nil?
    ...
    requirements = @room_html.xpath("/html/body/div[3]/div/div[1]/section/div[3]/p")[0].text
    description = @room_html.xpath("/html/body/div[3]/div/div[1]/section/div[4]/p")[0].text
    minutes = JSON.parse(result)["rows"][0]["elements"][0]["duration"]["value"].to_f/60
    resultTable << ["http://www.compartodepa.com.mx/#{newURL}", "%i:%i" % [minutes,(minutes-minutes.to_i)*60] ,minutes, price]
  end
end
